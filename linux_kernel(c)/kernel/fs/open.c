...
int do_open(const char *filename, int oflags)
{
        ...
        struct file *f = fget(-1);
        struct vnode *resvnode = NULL;
        int ret = 0;
        if ((ret = open_namev(filename, oflags, &resvnode, NULL)) != 0)
        {
                fput(f);
                dbg(DBG_PRINT, "(GRADING2B)\n");
                return ret;
        ...
        if ((oflags & O_RDWR) == O_RDWR)
        {
                f->f_mode = (f->f_mode | FMODE_READ | FMODE_WRITE);
                dbg(DBG_PRINT, "(GRADING2B)\n");
        }
        else if ((oflags & O_WRONLY) == O_WRONLY)
        {
                f->f_mode = (f->f_mode | FMODE_WRITE);
                dbg(DBG_PRINT, "(GRADING2B)\n");
        }
        else if ((oflags & O_RDONLY) == O_RDONLY)
        {
                f->f_mode = (f->f_mode | FMODE_READ);
                dbg(DBG_PRINT, "(GRADING2B)\n");
        }
