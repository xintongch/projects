...
{
        // dbg(DBG_TERM, "inside vget, with ino_t=%u\n", vno);
        vnode_t *vn = NULL;
...
find:
        ...
        {
                // dbg(DBG_TERM, "inside list iteration\n");
                if ((vn->vn_fs == fs) && (vn->vn_vno == vno))
                {
                        // dbg(DBG_TERM, "found one inuse vnode\n");
                        /* found it... */
        ...
        // dbg(DBG_TERM, "didn't find the vnode\n");
        /* if we got here, we didn't find the vnode. */
        /*   alloc a new vnode: */
        vn = slab_obj_alloc(vnode_allocator);
        // dbg(DBG_TERM, "the new allocated vnode=%p\n", vn);
        if (!vn)
        {
...
void vput(struct vnode *vn)
{
        ...
        if ((vn->vn_nrespages == (vn->vn_refcount - 1)) && !vn->vn_fs->fs_op->query_vnode(vn))
        {
                pframe_t *vp;
...
int vfs_is_in_use(fs_t *fs)
{
        ...
        for (link = list->l_next; link != list; link = link->l_next)
        {
                ...
                KASSERT(0 <= refs);
                KASSERT(((vn->vn_fs->fs_root == vn) && (1 <= refs)) || ((vn->vn_fs->fs_root != vn) && (0 <= refs)));

                if (((vn->vn_fs->fs_root == vn) && (1 < refs)) || ((vn->vn_fs->fs_root != vn) && refs))
                {
...
{
        dbg(DBG_TEMP, "inside init_special_vnode with nv=%p\n", vn);
...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
        ...
        bytesread = file->vn_cdev->cd_ops->read(file->vn_cdev, offset, buf, count);
        dbg(DBG_PRINT, "(GRADING2B)\n");
        // ((char *)buf)[bytesread - 1] = '\0';
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
        ...
        dbg(DBG_PRINT, "(GRADING2A 1.b)\n");
        file->vn_cdev = bytedev_lookup(file->vn_devid);
        byteswritten = file->vn_cdev->cd_ops->write(file->vn_cdev, offset, buf, count);
        dbg(DBG_PRINT, "(GRADING2B)\n");
