...
int addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        vmarea_t *vmarea = vmmap_lookup(p->p_vmmap, vfn);
        if (!vmarea)
        {
                return 0;
        }
        if ((perm & PROT_WRITE) == PROT_WRITE)
        {
                if (!((vmarea->vma_prot & PROT_WRITE) == PROT_WRITE))
                {
                        return 0;
                }
        }
        if ((perm & PROT_READ) == PROT_READ)
        {
                if (!((vmarea->vma_prot & PROT_READ) == PROT_READ))
                {
                        return 0;
                }
        }
        if ((perm & PROT_EXEC) == PROT_EXEC)
        {
                if (!((vmarea->vma_prot & PROT_EXEC) == PROT_EXEC))
                {
                        return 0;
                }
        }
...
{
        KASSERT(p);
        const char *curpos = avaddr;
        int curvfn = 0;
        vmarea_t *vmarea = NULL;

        while (curpos <= ((char *)avaddr + len))
        {
                curvfn = ADDR_TO_PN(curpos);
                vmarea = vmmap_lookup(p->p_vmmap, curvfn);
                if (!vmarea)
                {
                        return 0;
                }
                if (!addr_perm(p, curpos, perm))
                {
                        return 0;
                }
                curpos += PAGE_SIZE;
