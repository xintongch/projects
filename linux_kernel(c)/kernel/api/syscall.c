...
{
        int ret = 0;
        read_args_t kernelargs;
        char *buf = NULL;
        int nbytesread = 0;

        if ((ret = copy_from_user(&kernelargs, arg, sizeof(read_args_t))) != 0)
        {
                curthr->kt_errno = -ret;
                return -1;
        }
        ...
        if ((nbytesread = do_read(kernelargs.fd, buf, kernelargs.nbytes)) < 0)
        {
                page_free(buf);
                curthr->kt_errno = -nbytesread;
                return -1;
        }

        if ((ret = copy_to_user(arg->buf, buf, nbytesread)) != 0)
        {
                page_free(buf);
                curthr->kt_errno = -ret;
                return -1;
...
{
        write_args_t kernelargs;
        int ret = 0;
        char *buf = NULL;
        int nbyteswritten = 0;

        if ((ret = copy_from_user(&kernelargs, arg, sizeof(write_args_t))) != 0)
        {
                curthr->kt_errno = -ret;
                return ret;
        }
        ...
        if ((nbyteswritten = copy_from_user(buf, kernelargs.buf, kernelargs.nbytes)) < 0)
        {
                page_free(buf);
                curthr->kt_errno = -nbyteswritten;
                return nbyteswritten;
        }
        if ((nbyteswritten = do_write(kernelargs.fd, buf, kernelargs.nbytes)) < 0)
        {
                page_free(buf);
                curthr->kt_errno = -nbyteswritten;
                return nbyteswritten;
...
{
        getdents_args_t kernelargs;
        int ret = 0;
        int ndirent = 0;
        dirent_t dirent;
        int ndirentgot = 0;

        if ((ret = copy_from_user(&kernelargs, arg, sizeof(getdents_args_t))) != 0)
        {
                curthr->kt_errno = -ret;
                return ret;
        }
        ndirent = kernelargs.count / sizeof(dirent_t);
        for (int i = 0; i < ndirent; i++)
        {
                if ((ret = do_getdent(kernelargs.fd, &dirent)) < 0)
                {
                        curthr->kt_errno = -ret;
                        return ret;
                }
                ...
                if ((ret = copy_to_user(arg->dirp + i, &dirent, sizeof(dirent_t))) != 0)
                {
                        curthr->kt_errno = -ret;
                        return ret;
                }
        ...
        return ndirentgot * sizeof(dirent_t);
