...
int do_brk(void *addr, void **ret)
{
        ...
        int brkvfn = ADDR_TO_PN(curproc->p_brk);
        int vfnbeforebrk = ADDR_TO_PN((char *)(curproc->p_brk) - 1);
        int curvfn = ADDR_TO_PN(addr);
        int retval = 0;
        vmarea_t *vmarea = NULL;

        if (vfnbeforebrk != brkvfn)
        {
                brkvfn = vfnbeforebrk;
        }
        ...
        if (curvfn < brkvfn)
        {
                if ((retval = vmmap_remove(curproc->p_vmmap, curvfn, (brkvfn - curvfn + 1))) != 0)
                {
                        return -1;
                }
                *ret = curproc->p_brk;
        ...
        {
                if (!(vmarea = vmmap_lookup(curproc->p_vmmap, brkvfn)))
                {
                        return -1;
                }
                vmarea->vma_end = curvfn;
                *ret = addr;
        }
        return 0;
