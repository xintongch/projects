...
shadow_create()
{
        ...
        mmobj = slab_obj_alloc(shadow_allocator);
        // KASSERT(mmobj != NULL);
        if (mmobj)
        {
...
shadow_lookuppage(mmobj_t *o, uint32_t pagenum, int forwrite, pframe_t **pf)
{
        ...
        if (forwrite)
        {
                ...
                if (!pframe)
                {
                        if ((ret = pframe_get(o, pagenum, &pframe)) != 0)
                        {
                                return ret;
                        }
                        shadow_dirtypage(o, pframe);
                }
                *pf = pframe;
                return 0;
        }
        else
        {
                ...
                {
                        pframe = pframe_get_resident(curmmobj, pagenum);
                        if (pframe)
                        {
                                *pf = pframe;
                                return 0;
                ...
                return pframe_lookup(curmmobj, pagenum, forwrite, pf);
        }
...
shadow_fillpage(mmobj_t *o, pframe_t *pf)
{
        ...
        while (curmmobj->mmo_shadowed != NULL)
        {
                pframe = pframe_get_resident(curmmobj->mmo_shadowed, pf->pf_pagenum);
                if (pframe != NULL)
                {
                        pframe_pin(pf);
        ...
        // pframe = pframe_get_resident(curmmobj, pf->pf_pagenum);
        // if (pframe != NULL)
        // {
        //         pframe_pin(pf);
        //         memcpy(pf->pf_addr, pframe->pf_addr, PAGE_SIZE);
        //         return 0;
        // }
        if ((ret = pframe_lookup(o->mmo_un.mmo_bottom_obj, pf->pf_pagenum, 0, &pframe)) != 0)
        {
                return ret;
        }
        pframe_pin(pf);
...
shadow_cleanpage(mmobj_t *o, pframe_t *pf)
{
        ...
        if ((ret = o->mmo_ops->lookuppage(o, pf->pf_pagenum, 1, &pframe)) != 0)
        {
                return ret;
        }
