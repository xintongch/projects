...
{
        kthread_t *clonedthr = slab_obj_alloc(kthread_allocator);
        KASSERT(clonedthr != NULL);
        // memset(thr, 0, sizeof(kthread_t));
        clonedthr->kt_kstack = alloc_stack();
        clonedthr->kt_errno = thr->kt_errno;
        clonedthr->kt_retval = thr->kt_retval;
        clonedthr->kt_proc = NULL;
        clonedthr->kt_cancelled = thr->kt_cancelled;
        clonedthr->kt_wchan = thr->kt_wchan;
        clonedthr->kt_state = thr->kt_state;
        // context_setup(&(clonedthr->kt_ctx), NULL, 0, NULL, clonedthr->kt_kstack, DEFAULT_STACK_SIZE, thr->kt_proc->p_pagedir);
        context_setup(&(clonedthr->kt_ctx), NULL, 0, NULL, clonedthr->kt_kstack, DEFAULT_STACK_SIZE, curproc->p_pagedir);
        return clonedthr;
