...
// extern int getfilename(vnode_t *dir, const char *path);
...
proc_create(char *name)
{
        ...
        memset(p->p_files, 0, sizeof(struct file *) * NFILES);
        p->p_status = 0;
        p->p_brk = NULL;
        p->p_start_brk = NULL;
...
void proc_kill(proc_t *p, int status)
{
        ...
        kthread_t *thr = list_head(&(p->p_threads), kthread_t, kt_plink);
        p->p_status = status;
